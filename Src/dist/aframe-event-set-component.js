...
AFRAME.registerComponent('event-set', {
  ...
  updateEventListener: function updateEventListener() {
    ...
    if (data._target) {
      targetEl = el.sceneEl.querySelectorAll(data._target);
    } else if (data._subtarget) {
      targetEl = el.querySelectorAll(data._subtarget);
    } else {
      NodeList = false;
      targetEl = el;
    }
    ...
    var handler = function handler() {
      ...
      for (propName in data) {
        if (propName === '_event' || propName === '_target' || propName === '_subtarget') {
          continue;
        }

        if (NodeList) {
          for(i = 0; i < targetEl.length; i++)
            AFRAME.utils.entity.setComponentProperty.call(_this, targetEl[i], propName, data[propName]);
        } else {
